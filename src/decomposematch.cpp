...
WholeGenome::WholeGenome(int thread_num_,
    ...
    ref_variant_by_chrid = new std::vector<DiploidVariant>*[chrom_num];

	for (int j = 0; j < chrom_num; j++) {
		ref_variant_by_chrid[j] = new std::vector<DiploidVariant>;
	}

    que_variant_by_chrid = new std::vector<DiploidVariant>*[chrom_num];
    for (int j = 0; j < chrom_num; j++) {
        que_variant_by_chrid[j] = new std::vector<DiploidVariant>;
    }

    // chr_id starts from 0
	for(int j = 1; j <= 22; j++){
        std::string chr_name = to_string(j);
...
bool WholeGenome::ReadWholeGenomeSequence(std::string filename){
...
bool WholeGenome::ReadGenomeSequenceList(std::string filename){
...
int WholeGenome::ReadWholeGenomeVariant(std::string filename, bool flag){
    int total_num = 0;
    int long_num = 0;
	...
	while (!vcf_file.eof()) { // alternative way is vcf_file != NULL
		...
		std::string chr_name = columns[0];
		auto pos = atoi(columns[1].c_str()) - 1; // 0-based coordinate

		auto ref = columns[3];
		auto alt_line = columns[4];
		double quality = stod(columns[5]);
        std::string vcf_id_string = columns[6];
        
        int vcf_id = -1;
        if(is_digits(vcf_id_string)){
            vcf_id = stoi(vcf_id_string);
        }
        ...
        std::vector<std::string> genotype_columns;
        ...
        if(alt_list.size() > 2){
            if(match_mode_indicator != 1){
            std::vector<std::string> temp_alt_list = alt_list;
            alt_list.clear();
            for(int i = 0; i < 2; i++){
		...
		DiploidVariant dv(pos, ref, alt_list, is_heterozygous_variant, is_multi_alternatives, snp_del, snp_ins, flag, quality, is_zero_one_var, vcf_id);
		//if (normalization) {
    ...
    if(flag){
        ...
        std::vector<float> temp_percentage_list;
...
bool WholeGenome::ReadVariantFileList(std::string filename){
...
int WholeGenome::needleman_wunsch(std::string S1, std::string S2, std::string &R1, std::string &R2)
{
    int M = S1.length();
    int N = S2.length();
    /*
...
void WholeGenome::GenerateAltVector(std::string ref, std::string alt, std::vector<std::string> & alt_vector){
    if(ref.size() == 0) return;
    std::string ref_match = "";
    std::string alt_match = "";
...
bool WholeGenome::ParallelClustering(){
    // parallel by chr
    variant_cluster_by_chrid = new std::vector<std::vector<VariantIndicator>> *[chrom_num];
    for (int j = 0; j < chrom_num; j++) {
        variant_cluster_by_chrid[j] = new std::vector<std::vector<VariantIndicator>>;
    }
        ...
        for(int j = 0; j < thread_num-1 && chr_id < chrom_num-1; j++){
            if(chrname_by_chrid.find(chr_id) != chrname_by_chrid.end()){
                if(ref_variant_by_chrid[chr_id]->size() > 0 && que_variant_by_chrid[chr_id]->size() > 0){
                    threads.push_back(std::thread(&WholeGenome::SingleThreadClustering, this, chr_id));
                }
            }
            chr_id ++;
        }
...
bool WholeGenome::CheckTandemRepeat(std::string sequence, int unit_threshold) {
    ...
    for (int repeat_length = 1; repeat_length <= end_index; repeat_length++) {
        ...
        int repeat_time = 1;
        std::string repeat_region = sequence.substr(0, repeat_length);
        int start_position = repeat_length;
        while (start_position < sequence_length) {
            if (start_position + repeat_length > sequence_length)
                break;
            std::string matching_region = sequence.substr(start_position, repeat_length);
...
bool WholeGenome::MatchVariantListInThread(int thread_index, 
    int threshold_index,
    int chr_id,
    std::vector<DiploidVariant> & variant_list,
    int cluster_id){
    //===================================================
    sort(variant_list.begin(), variant_list.end());
    // decide reference sequence
    std::vector<DiploidVariant> separate_var_list[2];
    ...
    if (separate_var_list[0].size() == 1 && separate_var_list[1].size() == 1){
        ...
        if(separate_var_list[0][0] == separate_var_list[1][0]){
            ...
            std::string match_record = chrname_by_chrid[chr_id] + "\t" + to_string(tv.pos+1) + "\t" + tv.ref + "\t" + tv.alts[0];
            if(tv.multi_alts) match_record += "/" + tv.alts[1];
            match_record += "\t.\t.\t.\t.\t";
            match_record += to_string(separate_var_list[0][0].vcf_id) + "\t" + to_string(separate_var_list[1][0].vcf_id);
            match_record += "\t.\n";
    ...
    std::vector<bool> appliable_flag;
    ...
    std::string subsequence = genome_sequences[chr_id].substr(min_pos, max_pos - min_pos);
    ...
    std::vector<Interval> mergered_intervals = merge(intervals);
...
bool WholeGenome::ClusteringMatchInThread(int start, int end, int thread_index) {
	...
	for (int cluster_id = start; cluster_id < end; cluster_id++) {
        ...
        std::vector<VariantIndicator> vi_list = variants_by_cluster[cluster_id];
...
bool WholeGenome::PathMakeDecisionNoGenotype(SequencePath& sp,
                                 std::vector<DiploidVariant> & variant_list,
                                 multimap<int, int> * choices_by_pos[],
                                 std::list<SequencePath> & sequence_path_list,
                                 const std::string & reference_sequence,
                                 int score_unit,
                                 int match_mode,
                                 int score_scheme)
{
    ...
    std::vector<pair<int, int>> candidate_choices[2];
    for(int i = 0; i < 2; i++){
        ...
        for(auto it = var_range.first; it != var_range.second; ++it){
            ...
            std::string ref = var.ref; //even we do not know the offset, we know ref start from pos of reference_sequence
            std::string alts[2];
            alts[0] = var.alts[0];
            alts[1] = alts[0];
        ...
        for(int j = 0; j < candidate_choices[1].size(); j++){
            ...
            for(int x = 0; x < 2; x++){
                ...
                if(var_index != -1){
                    ...
                    if(k < alts[y].length()){
                        std::string alt_part = alts[y].substr(k, alts[y].length()-k);
                         ...
                         int c)
{
    ...
    for(int y = 0; y < 2; y++){
        ...
        std::string alt_part = alt_vector[k];
...
bool WholeGenome::MatchingSingleClusterBaseExtending(int cluster_index,
    ...
    std::list<SequencePath> current_path_list;
    std::list<SequencePath> next_path_list;
...
void WholeGenome::ConstructMatchRecord(SequencePath & best_path,
    ...
    std::string parsimonious_ref = subsequence;
    std::string parsimonious_alt0 = best_path.donor_sequences[0];
    std::string parsimonious_alt1 = best_path.donor_sequences[1];
...
//                             chr_id);
    ...
    std::string match_record = chrname_by_chrid[chr_id] + "\t" + to_string(parsimonious_pos+1) + "\t" + parsimonious_ref + "\t" + parsimonious_alt0;
    if(multiple_match) match_record += "/" + parsimonious_alt1;

    std::string vcf_record[2];
    std::string phasing_record[2];
    std::string vcf_id_record[2];
    ...
    for (int i = 0; i < 2; i++) {
        for (int var_index = 0; var_index < variant_list.size(); var_index++) {
            ...
            if(need_match_record){
                std::string alt_string = variant.alts[0];
                if(variant.multi_alts){
                    alt_string += "/" + variant.alts[1];
                }
                std::string phasing_string = "";
                if(phasing == 0){
                    phasing_string += "1";
                ...
                std::string variant_record = to_string(variant.pos+1) + "," + variant.ref + "," + alt_string;
                std::string vcf_id_string = to_string(variant.vcf_id);
                vcf_record[i] += variant_record;
                phasing_record[i] += phasing_string;
                vcf_id_record[i] += vcf_id_string;
                vcf_record[i] += ";";
                phasing_record[i] += ";";
                vcf_id_record[i] += ";";
            }
        ...
        if(need_match_record){
            ...
            vcf_id_record[i] = vcf_id_record[i].substr(0, vcf_id_record[i].size()-1);
        }
    ...
    if(need_match_record){
        ...
        match_record += "\t" + vcf_id_record[0] + "\t" + vcf_id_record[1];
...
void WholeGenome::ConstructMatchRecordNoGenotype(SequencePath & best_path,
    ...
    std::string parsimonious_alt0 = best_path.donor_sequences[0];
    std::string parsimonious_alt1 = best_path.donor_sequences[0];
...
void WholeGenome::ConvergePaths(std::list<SequencePath> & path_list){
    //dout << "===========start converge===================" << endl;
    int path_num = path_list.size();
    if(path_num <= 1) return;
    for(std::list<SequencePath>::iterator i = path_list.begin(); i!= path_list.end(); ++i){
        SequencePath  ref_path = *i;
        if(ref_path.removable) continue;
        if(!ref_path.same_donor_len) continue;
        std::list<SequencePath>::iterator j = i;
        ++j;
...
int WholeGenome::test() {
	...
	DiploidVariant d1(1, "T", std::vector<std::string> ({"A", "C"}), true, true, 0,0,0);
	DiploidVariant d2(4, "G", std::vector<std::string> ({"C", ""}), true, false, 0,0,0);
	DiploidVariant d3(5, "C", std::vector<std::string> ({"T", ""}), true, false, 0,0,0); // this is false negative
	DiploidVariant d4(6, "C", std::vector<std::string> ({"G", ""}), true, false, 0,0,0);
	DiploidVariant d5(7, "G", std::vector<std::string> ({"A", ""}), true, false, 0,0,0);
	DiploidVariant d6(1, "T", std::vector<std::string> ({"A", "C"}), true, true, 0,0,1);
	DiploidVariant d7(3, "AG", std::vector<std::string> ({"A", ""}), true, false, 1,0,1);
	DiploidVariant d8(7, "G", std::vector<std::string> ({"GA", ""}), true, false, 0,1,1);

    //complex_match_records = new std::vector<std::string>*[1];
    //complex_match_records[0] = new std::vector<std::string>;
	//std::vector<DiploidVariant> var_list = { d2,d3,d4,d5,d7,d8 };
	std::vector<DiploidVariant> var_list = { d1,d2,d3,d4,d5,d6,d7,d8 };
...
void WholeGenome::ClusteringMatchMultiThread() {
	...
	//complex_match_records = new vector<std::string>*[thread_num];
	match_records_by_mode_by_thread = new std::vector<std::string>**[thread_num];

    //query_matches_by_mode_by_thread = new std::vector<int> ** [thread_num];

	for(int i = 0; i < thread_num; i++){
        match_records_by_mode_by_thread[i] = new std::vector<std::string>*[MATCH_MODE_NUM];
        for(int j = 0; j < MATCH_MODE_NUM; j++){
            match_records_by_mode_by_thread[i][j] = new std::vector<std::string>;
        }
	}

    baseline_total_match_num = new std::vector<int>** [thread_num];
    query_total_match_num = new std::vector<int> ** [thread_num];

    baseline_total_edit_distance = new std::vector<int>** [thread_num];
    query_total_edit_distance = new std::vector<int>** [thread_num];

    for(int i = 0; i < thread_num; i++){
        
        baseline_total_match_num[i] = new std::vector<int>* [ROC_SAMPLE_NUM];
        query_total_match_num[i] = new std::vector<int>* [ROC_SAMPLE_NUM];

        baseline_total_edit_distance[i] = new std::vector<int> * [ROC_SAMPLE_NUM];
        query_total_edit_distance[i] = new std::vector<int>* [ROC_SAMPLE_NUM];

        for(int j = 0; j < ROC_SAMPLE_NUM; j++){
            baseline_total_match_num[i][j] = new std::vector<int>;
            baseline_total_match_num[i][j]->resize(MATCH_MODE_NUM, 0);
            query_total_match_num[i][j] = new std::vector<int>;
            query_total_match_num[i][j]->resize(MATCH_MODE_NUM, 0);

            baseline_total_edit_distance[i][j] = new std::vector<int>;
            baseline_total_edit_distance[i][j]->resize(MATCH_MODE_NUM, 0);
            query_total_edit_distance[i][j] = new std::vector<int>;
	...
	for (; i < thread_num - 1; i++) {
		threads.push_back(std::thread(&WholeGenome::ClusteringMatchInThread, this, start, end, i));
		start = end;
    ...
    std::string stat_head_string = "#score_unit\tmatch_mode\tscore_unit\tqual_threshold\tbaseline_match_num\tquery_match_num\tquery_total_num\tbaseline_total_ED\tquery_total_ED";
    ...
    for(int x = 0; x < score_unit_list.size(); x++){
        ...
        for(int y = 0; y < match_mode_list.size(); y++){
            ...
            for(int z = 0; z < score_scheme_list.size(); z++){
                ...
                std::string baseline_match_num_string = "";
                std::string query_match_num_string = "";
                std::string query_total_num_string = "";

                std::string baseline_edit_distance_string = "";
                std::string query_edit_distance_string = "";
                ...
                std::string total_match_num_string = to_string(score_unit) + "\t" +
    ...
    for(int x = 0; x < score_unit_list.size(); x++){
        ...
        for(int y = 0; y < match_mode_list.size(); y++){
            ...
            for(int z = 0; z < score_scheme_list.size(); z++){
                score_scheme = score_scheme_list[z];
                int mode_index = GetIndexFromMatchScore(score_unit, match_mode, score_scheme);
                std::string filename_index = to_string(score_unit) + "_" + to_string(match_mode) + "_" + to_string(score_scheme);
                ...
                output_complex_file << "#CHROM\tPOS\tREF\tALT\tVCF1\tVCF2\tPHASE1\tPHASE2\tVCFID1\tVCFID2\tSCORE" << endl;
...
//[TODO] unit test
int WholeGenome::NormalizeVariantSequence(int pos, std::string & parsimonious_ref, std::string & parsimonious_alt0, std::string & parsimonious_alt1, int chr_id) {
...
void WholeGenome::SingleThreadClustering(int chr_id) {
    ...
    DiploidVariant snp;
    std::vector<VariantIndicator> vi_list;
    while (ref_index < ref_size || que_index < que_size) {
		...
		if (not_first) {
			...
			if (c_end - c_start >= 2) {
				...
				std::string separator = genome_sequences[chr_id].substr(c_start, separator_length);
...
int WholeGenome::ReadReferenceVariants(std::string filename){
    return ReadWholeGenomeVariant(filename, false);
}

int WholeGenome::ReadQueryVariants(std::string filename){
    return ReadWholeGenomeVariant(filename, true);
}

void WholeGenome::ReadRef(std::string genome_seq, std::string ref_vcf){
...
void WholeGenome::Compare(std::string query_vcf,
...
void WholeGenome::DirectMatch(std::string ref_vcf, std::string query_vcf, int match_mode_, std::string output_prefix)
{
        ...
        for(int j = 0; j < que_variant_by_chrid[i]->size(); j++){
            ...
            for(auto it = var_range.first; it != var_range.second; ++it){
                ...
                if (match_mode_indicator != 1 && var == ref_var){
                    match_num ++;
                    std::string matched_variant = chrname_by_chrid[i] + "\t" + to_string(ref_var.pos) + "\t" + to_string(ref_var.vcf_id) + "\t" + to_string(var.pos) + "\t" + to_string(var.vcf_id);
