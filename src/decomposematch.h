...
public:
    ...
    std::vector<std::string> string_sequences[4];
    std::map<int, std::pair<int, int>> choice_made[2]; // this can be used to indicate if choice is made and which choice
    // one choice is a pair: variant id, phasing index
    int current_genome_pos;
    std::string donor_sequences[4];
...
private:
    ...
    std::vector<std::string> baseline_variant_strings;
    std::vector<std::string> query_variant_strings;
    bool detail_results;

    //int thread_num; VCF->DiploidVariant->WholeGenome
protected:
    std::map<std::string, int> chrid_by_chrname;
    std::map<int, std::string> chrname_by_chrid;
    std::map<std::string, int> chrname_dict;
    std::map<int, std::string> genome_sequences;
    std::vector<DiploidVariant> ** ref_variant_by_chrid;
    std::vector<DiploidVariant> ** que_variant_by_chrid;
    std::vector<std::vector<VariantIndicator>> ** variant_cluster_by_chrid;
    ...
    std::vector<std::vector<VariantIndicator>> variants_by_cluster;

    std::vector<std::string> *** match_records_by_mode_by_thread;

    //vector<int> *** baseline_matches_by_mode_by_thread;
    //vector<int> *** query_matches_by_mode_by_thread;
    std::vector<int> *** baseline_total_match_num;
    std::vector<int> *** query_total_match_num;

    std::vector<int> *** baseline_total_edit_distance;
    std::vector<int> *** query_total_edit_distance;
    ...
    std::vector<int> score_unit_list;
    std::vector<int> match_mode_list;
    std::vector<int> score_scheme_list;
    std::vector<int> mode_index_list;

    std::vector<double> threshold_list;
    int threshold_num;

    std::vector<float> per_list;

    bool ReadWholeGenomeSequence(std::string filename);
    bool ReadGenomeSequenceList(std::string filename);
    int ReadWholeGenomeVariant(std::string filename, bool flag);
    bool ReadVariantFileList(std::string filename);
    int ReadReferenceVariants(std::string filename);
    int ReadQueryVariants(std::string filename);
    ...
    int NormalizeVariantSequence(int pos,
                             std::string & parsimonious_ref,
                             std::string & parsimonious_alt0,
                             std::string & parsimonious_alt1,
                             int chr_id);
    ...
    int PathNeedDecision(SequencePath& sp, std::multimap<int, int> * choices_by_pos[], int pos);
    int PathExtendOneStep(SequencePath& sp,
                          std::multimap<int, int> * choices_by_pos[],
    ...
    void ConvergePaths(std::list<SequencePath> & path_list);
    ...
    bool CheckTandemRepeat(std::string sequence, int unit_threshold);
    ...
    int needleman_wunsch(std::string S1, std::string S2, std::string &R1, std::string &R2);
    void GenerateAltVector(std::string ref, std::string alt, std::vector<std::string> & alt_vector);
...
public:
    ...
    void ReadRef(std::string genome_seq, 
      std::string ref_vcf);

    void Compare(std::string query_vcf,
        std::string output_prefix,
        bool detail_results,
        int score_unit_,
        int match_mode_,
    ...
    void DirectMatch(std::string ref_vcf,
                std::string query_vcf,
                int match_mode_,
                std::string output_prefix);
